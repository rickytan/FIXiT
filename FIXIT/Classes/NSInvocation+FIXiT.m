...
- (void)fixit_setArgument:(id)value atIndex:(NSUInteger)index
{
    ...
    // Skip const type qualifier.
    if (argType[0] == _C_CONST) argType++;
#define READ_AND_SET(type, method) do { type val = [obj method]; [self setArgument:&val atIndex:(NSInteger)index]; } while (0)
    if (strcmp(argType, @encode(id)) == 0 || strcmp(argType, @encode(Class)) == 0) {
        ...
        READ_AND_SET(char, charValue);
    } else if (strcmp(argType, @encode(int)) == 0) {
        READ_AND_SET(int, intValue);
    } else if (strcmp(argType, @encode(short)) == 0) {
        READ_AND_SET(short, shortValue);
    } else if (strcmp(argType, @encode(long)) == 0) {
        READ_AND_SET(long, longValue);
    } else if (strcmp(argType, @encode(long long)) == 0) {
        READ_AND_SET(long long, longLongValue);
    } else if (strcmp(argType, @encode(unsigned char)) == 0) {
        READ_AND_SET(unsigned char, unsignedCharValue);
    } else if (strcmp(argType, @encode(unsigned int)) == 0) {
        READ_AND_SET(unsigned int, unsignedIntValue);
    } else if (strcmp(argType, @encode(unsigned short)) == 0) {
        READ_AND_SET(unsigned short, unsignedShortValue);
    } else if (strcmp(argType, @encode(unsigned long)) == 0) {
        READ_AND_SET(unsigned long, unsignedLongValue);
    } else if (strcmp(argType, @encode(unsigned long long)) == 0) {
        READ_AND_SET(unsigned long long, unsignedLongLongValue);
    } else if (strcmp(argType, @encode(float)) == 0) {
        READ_AND_SET(float, floatValue);
    } else if (strcmp(argType, @encode(double)) == 0) {
        READ_AND_SET(double, doubleValue);
    } else if (strcmp(argType, @encode(BOOL)) == 0) {
        READ_AND_SET(BOOL, boolValue);
    } else if (strcmp(argType, @encode(bool)) == 0) {
        READ_AND_SET(BOOL, boolValue);
    } else if (strcmp(argType, @encode(char *)) == 0) {
        READ_AND_SET(const char *, pointerValue);
    } else if (strcmp(argType, @encode(void (^)(void))) == 0) {
...
- (void)fixit_setReturnValue:(JSValue *)value
{
    ...
    // Skip const type qualifier.
    if (argType[0] == _C_CONST) argType++;
#define READ_AND_SET(type, method) do { type val = [value method]; [self setReturnValue:&val]; } while (0)
    if (strcmp(argType, @encode(id)) == 0 || strcmp(argType, @encode(Class)) == 0) {
        ...
        READ_AND_SET(char, toInt32);
    } else if (strcmp(argType, @encode(int)) == 0) {
        READ_AND_SET(int, toInt32);
    } else if (strcmp(argType, @encode(short)) == 0) {
        READ_AND_SET(short, toInt32);
    } else if (strcmp(argType, @encode(long)) == 0) {
        READ_AND_SET(long, toInt32);
    } else if (strcmp(argType, @encode(long long)) == 0) {
        READ_AND_SET(long long, toDouble);
    } else if (strcmp(argType, @encode(unsigned char)) == 0) {
        READ_AND_SET(unsigned char, toUInt32);
    } else if (strcmp(argType, @encode(unsigned int)) == 0) {
        READ_AND_SET(unsigned int, toUInt32);
    } else if (strcmp(argType, @encode(unsigned short)) == 0) {
        READ_AND_SET(unsigned short, toUInt32);
    } else if (strcmp(argType, @encode(unsigned long)) == 0) {
        READ_AND_SET(unsigned long, toUInt32);
    } else if (strcmp(argType, @encode(unsigned long long)) == 0) {
        READ_AND_SET(unsigned long long, toDouble);
    } else if (strcmp(argType, @encode(float)) == 0) {
        READ_AND_SET(float, toDouble);
    } else if (strcmp(argType, @encode(double)) == 0) {
        READ_AND_SET(double, toDouble);
    } else if (strcmp(argType, @encode(BOOL)) == 0) {
        READ_AND_SET(BOOL, toBool);
    } else if (strcmp(argType, @encode(bool)) == 0) {
        READ_AND_SET(BOOL, toBool);
    } else if (strcmp(argType, @encode(char *)) == 0) {
